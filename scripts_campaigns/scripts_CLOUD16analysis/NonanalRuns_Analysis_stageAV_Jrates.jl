...
sc=scatter(runplan[(.!(dark) .& GCR .& lowT),"H2SO4 [cm⁻³] online data"],
    ...
    vmin=5, vmax=8,cmap=cm,marker = "^",edgecolors="dimgrey", label = "GCR", s=100)
scatter(runplan[(.!(dark) .& Neutral .& lowT),"H2SO4 [cm⁻³] online data"],
    ...
    vmin=5, vmax=8,cmap=cm, marker="o",edgecolors="dimgrey", label = "Neutral", s=100)
scatter(runplan[(.!(dark) .& Beam .& lowT),"H2SO4 [cm⁻³] online data"],
        ...
        vmin=5, vmax=8,cmap=cm, marker="D",edgecolors="dimgrey",label = "Beam", s=100)
scatter(runplan[(LS1on .& Beam .& lowT),"H2SO4 [cm⁻³] online data"],
        ...
        vmin=5, vmax=8,cmap=cm, marker="D", edgecolors="orange", label = "Beam, LS1on", s=100)
scatter(runplan[(LS1on .& Neutral .& lowT),"H2SO4 [cm⁻³] online data"],
        ...
        vmin=5, vmax=8,cmap=cm, marker="o", edgecolors="orange", label = "Neutral, LS1on", s=100)
scatter(runplan[(LS1on .& GCR .& lowT),"H2SO4 [cm⁻³] online data"],
        ...
        vmin=5, vmax=8,cmap=cm, marker="^", edgecolors="orange", label = "GCR, LS1on", s=100)
CLOUD18_runplan = DataFrame(stage=[3207.04,3207.09,3207.11,3207.201,
                            3207.202,3208.01,3208.03,3208.06,3208.09],
                            J25=[6,14,15.6,69,12,9.1,2.8,5.9,0.2],
                            SA=[6e6,6e6,5e6,2e5,2e5,2.3e6,1.8e6,2.2e6,2e5],
                            HOM=[1e4,4e5,7e5,5.7e6,1.5e6,1.8e6,7.4e5,1.2e6,1.9e6].*2.5
                            )
# -15°C
CLOUD18_runplan_plus10 = DataFrame(stage=[3205.02,3206.01,3206.03,3206.06,3206.10,3206.12],
                            J25=[3.7,5,0.09,0.017,25,28],
                            SA=[5e6,4e6,1e6,9e5,1.5e7,1.7e7],
                            HOM=[6e6,7e6,8e6,6e6,4.5e6,6.1e6].*2.5
                            )


                            scatter(CLOUD18_runplan.SA, CLOUD18_runplan.J25, c=log10.(CLOUD18_runplan.HOM), s=200,
        vmin=5, vmax=8,cmap=cm, marker="*", edgecolors="black", label = "CLOUD18, GCR")
for (i, txt) in enumerate(CLOUD18_runplan.stage)
    annotate(txt, (CLOUD18_runplan.SA[i], CLOUD18_runplan.J25[i]))
end
cb = colorbar(sc, label = "log10(LTOF_C9_C18 (online data) [cm⁻³])")
xscale("log")
yscale("log")
...
cb = colorbar(sc, label = "LTOF_C9_C18 (online data)")
xscale("log")
yscale("log")
xlabel("H2SO4 [cm⁻³] (online data)")
...
errorbar(xArr[(LS1on .& GCR .& lowT.& .!(isnan.(yArr)) .& .!(isnan.(xArr)) .& .!(isnan.(cArr)))],
    ...
    =#
cb = colorbar(sc, label = "summed ULVOCs [cm⁻³]")
...
cb = colorbar(sc, label = "sulfuric acid [cm⁻³]")
