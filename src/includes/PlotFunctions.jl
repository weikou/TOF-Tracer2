...
=#
	...
	function plotTraces(measResultOriginal::ResultFileFunctions.MeasurementResult,
		...
		PythonPlot.title(string("$startTimeString - $endTimeString", " ", title))
...
############################
    ...
    function InteractivePlot(file::String,ax::PythonCall.Py, fig::Figure)
	   	figure = fig
	   	axes = ax
	   	file = file
	   	lastPlottedIndex = 1
	...
	function InteractivePlot(file::String;useAveragesOnly=true)
		figure = PythonPlot.figure()
